...
/// Centralized sound effects. Buffers are synthesized once on first use and
/// replayed through a shared `AVAudioPlayerNode` so each call is allocation-
/// free and stays tight to the haptic that fires alongside it.
enum SoundEffects {
    private static let engine = AVAudioEngine()
    private static let player = AVAudioPlayerNode()
    private static let sampleRate: Double = 44_100
    private static var configured = false
    private static var logChimeBuffer: AVAudioPCMBuffer?
    ...
    /// Bright two-note bell that fires when a habit is logged.
    static func logChime() {
        configure()
        guard let buffer = logChimeBuffer else { return }
    ...
    private static func configure() {
        ...
        // switch is off. A habit log is a deliberate user action and should
        // be heard. mixWithOthers keeps any music or podcast playing through.
        ...
        logChimeBuffer = renderLogChime(format: format)
    }
    ...
    private static func renderLogChime(format: AVAudioFormat) -> AVAudioPCMBuffer? {
